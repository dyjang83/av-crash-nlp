...
1. **Fetch** (needs open internet): `python3 -m src.fetch.fetch_sgo` and
   `python -m src.fetch.fetch_dmv_ol316`. If your environment blocks these domains,
   run this step elsewhere and copy `data/raw/` over.
2. **Parse**: `python -m src.fetch.parse_ol316` → `data/interim/narratives.jsonl`.
3. **Sample gold**: `python -m src.annotate.make_gold_sample --n 250 --seed 11`.
